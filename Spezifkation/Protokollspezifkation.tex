...
\begin{lstlisting}
{
	...
	"admin": false,
	"status": "Verified",
	"registrationdate": 1323165456000,
	"verifieddate": null,
}
\end{lstlisting}
	...
	``registrationdate'' und ``verifieddate'' sind GMT-Timestamps in Millisekunden oder null. 
...
\end{lstlisting}
	...
	``request'' und ``response'' sind JSON-Objekte.
	
	``costs'' sind die Kosten als ganzahliger Wert in Cents.

	``requestdate'' und ``finisheddate'' sind GMT-Timestamps in Millisekunden oder null. 
	
	``duration'' ist die Berechnungsdauer des Requests in Millisekunden.
	
	``faildescription'' ist ein String oder null.
