...
\usecase{Nutzer registrieren}{Client, Benutzer-DB}%Name und Akteure
{Ein Nutzer soll in der Benutzer-DB registriert werden}%Ziel
{%Vorbedingungen
  \item Der Server ist in Betrieb.
...
\usecase{Nutzer Anmelden}{Client, Benutzer-DB}%Name und Akteure
...
\usecase{Nutzerdaten abfragen}{Client, Benutzer-DB}%Name und Akteure
{Client erhält seine Nutzerdaten}%Ziel
{%Vorbedingungen
  \item Der Server ist in Betrieb.
...
%TODO: schauen, ob wir das wollen
%\usecase{Nutzerdaten verändern}{Client, Benutzer-DB}%Name und Akteure
%{Client verändert seine Nutzerdaten}%Ziel
%{%Vorbedingungen
%  \item Der Server ist in Betrieb.
%  \item Der Server hat einen Request über \textit{/TODO} vom Client empfangen
%}
%{%Nachbedingungen
%  \item Client hat die Nutzerdaten verändert.
%}
%{%Normalablauf
%  \item Der Server prüft Benutzername und Passwort anhand der Benutzer-DB
%  \item Nutzerdaten werden an Client geschickt.
...
\usecase{Billinginformationen Abfragen}{Client, Benutzer-DB}%Name und Akteure
...
Webanwendungen, die irgendwelche Daten speichert, die man als Graph interpretieren kann, sind nicht zwangsläufig als graphbasierte Webanwendungen zu bezeichnen. Die Daten und deren Interpretation als Graph müssen Kernaspekte der angebotenen Dienste sein, damit die Webanwendung als graphbasiert bezeichnet werden kann.
}{%Gültigkeit
-
}{%Bezeichnung
-
...
Eine Anwendung, die mit einem Internetbrowser über das Internet zugänglich ist und mit einem Server kommuniziert.
}{%Abgrenzung
Sonstige Anwendungen, die über das Internet mit einem Server kommunizieren, wie zum Beispiel Apps, zählen nicht als Webanwendungen.
}{%Gültigkeit
-
}{%Bezeichnung
-
...
Die Abrechnungs-DB (Abrechnungs-Datenbank) speichert alle empfangenen relevanten Requests (Anfragen) von Clients und alle relevanten Responds (Antworten) des Servers, die für die Abrechnung benötigt werden.
}{%Abgrenzung
