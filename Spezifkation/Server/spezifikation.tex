...
\def\ifNotEmpty#1{\def\@temp{#1}\iffalse\@temp\@empty}
...
\hline
  ...
  Normalablauf & \vspace{-7pt}
  	\begin{owncompactenum}[1.] #7 \end{owncompactenum} \\\hline
  Nachbedingungen & \novspace
  	\begin{owncompactitem}[-] #5 \end{owncompactitem} \\\hline
  Sonderfall & #8\\\hline
  %\ifNotEmpty{#8}{Sonderfall & #8\\\hline}\fi
  Nachbedingung im Sonderfall& #6\\\hline
  %Makro doesn't work yet for last row
  %\ifEmpty{#6}{\end{tabular}}\else{Nachbedingung im Sonderfall& #6\\\hline \end{tabular}}\fi
\end{tabular}
